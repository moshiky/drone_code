...
const float LOOP_SEC_PART = LOOP_DELAY / 1000.0;
...
float lastXrot, lastYrot, lastZrot;

float currXangl, currYangl;
float xShiftMs, yShiftMs;
...
void setup() {
  ...
  xShiftMs = 10.0 / (25.782 * 1000);
  yShiftMs = 10.0 / (80.0 * 1000);
...
//  printData();
  ...
  currXangl += - (((rotY + lastYrot) / 2) * LOOP_SEC_PART);
  currYangl += (((rotX + lastXrot) / 2) * LOOP_SEC_PART);

  lastXrot = rotX;
  lastYrot = rotY;

  Serial.print("x= "); Serial.print(currXangl);  Serial.print("\t");
  Serial.print("y= "); Serial.print(currYangl);  Serial.println(" ");
