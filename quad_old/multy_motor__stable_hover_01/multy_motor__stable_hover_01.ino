...
void loop() {
  ...
  if (0 == start_stage) {
    ...
    if (thr_value > 15) {
      ...
      // set start stage to activated mode
      start_stage = 3;
    }
  }
  else {
    if (thr_value < 10) {
      ...
      // map read values to axis diffs
      int max_change = 10;
      // x_acc_diff > 0 if drone is tilting right, if left- x_acc_diff < 0
      unsigned int x_acc_diff = map(current_y_state, <read_min>, <read_max>, -max_change, max_change);
      // y_acc_diff > 0 if drone's front is above horizontal line, if below- y_acc_diff < 0
      unsigned int y_acc_diff = map(current_x_state, <read_min>, <read_max>, -max_change, max_change);
